$(function () {
    ...
    function setHeader(msg){
        if (msg.is_login) {  // 登录状态
            ...
            $("#nav-right li:eq(1) a").attr({"href":"/org/upload.html"})
            $("#nav-right li:eq(2) a").attr({"title": msg.user_name, "href": "/user/info.html?id=" + msg.user_id, "target": "_blank" });
            $("#nav-right li:eq(2) a img").attr({"src": app_root + msg.media_url + msg.head_photo})
            $("#nav-right li:eq(3) a").attr({"href": "javascript:void(0)", "id": "logout" }).text("退出登录");
            $("#nav-right li:eq(4) a").attr({"href": "javascript:void(0)", "id": "logoff" }).text("注销");
        }
        else {  // 游客状态
            ...
            $("#nav-right li:eq(3) a").removeAttr("id").attr({ "href": "/user/login.html", "target": "_self" }).text("登录");
            $("#nav-right li:eq(4) a").removeAttr("id").attr({ "href": "/user/register.html", "target": "_self" }).text("注册");
        }
    }
